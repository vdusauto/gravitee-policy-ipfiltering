...
public class IPFilteringPolicyConfigurationTest {
        ...
        public void testConfiguration_previousConfVersion() throws IOException {
            ...
            Assert.assertEquals(1, configuration.getWhitelistIps().size());
            Assert.assertEquals(1, configuration.getWhitelistIps().size());
            Assert.assertNull(configuration.getWhitelistIps().get(0).getInfo());
            Assert.assertNull(configuration.getWhitelistIps().get(0).getInfo());
        }
        ...
        public void testConfiguration_newConfVersion() throws IOException {
            ...
            Assert.assertEquals(2, configuration.getBlacklistIps().size());
            Assert.assertEquals(2, configuration.getWhitelistIps().size());
            Assert.assertEquals("TBD", configuration.getBlacklistIps().get(0).getInfo());
            Assert.assertEquals("TBD", configuration.getWhitelistIps().get(0).getInfo());
            Assert.assertEquals("IP_black1", configuration.getBlacklistIps().get(0).getIpOrCIDR());
            Assert.assertEquals("IP_white1", configuration.getWhitelistIps().get(0).getIpOrCIDR());
        }
